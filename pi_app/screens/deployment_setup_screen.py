...
class DeploymentSetupScreen(Screen):
    ...
    def reset_screen(self, is_reroll=False):
        ...
        if not is_reroll:
            ...
            self.status_label.text = "Players roll to determine Attacker/Defender."
        else:
            self.status_label.text = "It's a tie! Re-roll."
    ...
    def determine_winner(self):
        ...
        if self.p1_roll > self.p2_roll:
            self.winner_id = 1
            winner_name = self.p1_name_label.text
            self.status_label.text = f"{winner_name} wins! Choose role:"
            self.show_choice_buttons()
        elif self.p2_roll > self.p1_roll:
            self.winner_id = 2
            winner_name = self.p2_name_label.text
            self.status_label.text = f"{winner_name} wins! Choose role:"
            self.show_choice_buttons()
        else:
            # Tie
    ...
    def select_role(self, role):
        ...
        app = App.get_running_app()
        if self.winner_id == 1:
            attacker_name = self.p1_name_label.text if role == 'attacker' else self.p2_name_label.text
            defender_name = self.p2_name_label.text if role == 'attacker' else self.p1_name_label.text
        else: # winner_id == 2
            attacker_name = self.p2_name_label.text if role == 'attacker' else self.p1_name_label.text
            defender_name = self.p1_name_label.text if role == 'attacker' else self.p2_name_label.text
